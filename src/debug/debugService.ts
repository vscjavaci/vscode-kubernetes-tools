...
export class DebugService implements IDebugService {
    ...
    public async attachDebug(workspaceFolder: vscode.WorkspaceFolder, pod?: string): Promise<void> {
        ...
        const portInfo = await this.debugProvider.getDockerResolver().resolvePortsFromContainer(this.kubectl, targetPod, targetContainer);
        if (!portInfo || !portInfo.debug) {
            vscode.window.showErrorMessage("Cannot resolve debug port from container.");
            return;
        }
        ...
        vscode.window.withProgress({ location: vscode.ProgressLocation.Window }, async (p) => {
            try {
                // Setup port-forward.
                p.report({ message: "Creating port-forwarding..."});
                kubeChannel.showOutput(`Starting to create port-forward on pod ${targetPod}...`, "port-forward");
                ...
                await this.startDebugging(workspaceFolder.uri.fsPath, sessionName, proxyResult.proxyDebugPort, proxyResult.proxyAppPort, async () => {
                    proxyResult.proxyProcess.kill();
                });
            } catch (error) {
                // do nothing.
